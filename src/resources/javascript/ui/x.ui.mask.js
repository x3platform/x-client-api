...
define(['jquery', 'x'], function($, x)
{
    ...
    * @description 遮罩管理
    * @requires module:x
    * @requires module:x.drag
    * @requires module:x.util
    */
    x.ui.mask = {
        ...
        newMaskWrapper: function(name, options)
        {
            var maskWrapper = {
                ...
                show: function()
                {
                    ...
                    if (this.autoHide === 1)
                    {
                        $(wrapper).bind('click', function(event)
                        {
                            ...
                            if (x.form.query(mask.name).css('display') === '')
                            {
                    ...
                    if (wrapper.style.display === 'none')
                    {
                        ...
                        x.form.query(this.name).css({ display: '', opacity: 0.1 });

                        x.form.query(this.popupWindowName).css({ display: 'none' });

                        x.form.query(this.name).fadeTo((this.maxDuration * 1000), this.maxOpacity, function()
                        {
                            var mask = window[this.id];

                            // x.debug.log(mask.popupWindowName + '.show()');

                            x.form.query(mask.popupWindowName).css({ display: '' });
                            x.form.query(mask.popupWindowName).slideDown('slow');
                        });
                    }
                },
                ...
                */
                hide: function()
                {
                    if (x.form.query(this.popupWindowName).css('display') !== 'none')
                    {
                        /*
                        x.form.query(this.popupWindowName).css({ display: 'none' });

                        x.form.query(this.name).css({ display: '', opacity: this.maxOpacity });

                        x.form.query(this.name).fadeOut((this.maxDuration * 1000), function()
                        {
                        x.form.query(this.name).css({ display: 'none' });
                        });
                        */
                        ...
                        */
                        ...
                        x.form.query(this.popupWindowName).fadeOut('normal', function()
                        {
                            x.form.query(that.name).css({ display: '', opacity: that.maxOpacity });

                            x.form.query(that.name).fadeOut((that.maxDuration * 1000), function()
                            {
                                x.form.query(that.name).css({ display: 'none' });
                            });
                        });
                    }
                },
                ...
                */
                resize: function()
                {
                    var element = x.form.query(this.popupWindowName);
