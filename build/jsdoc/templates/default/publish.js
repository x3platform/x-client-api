...
function buildNav(members) {
    ...
    if (members.namespaces.length) {
        nav += '<h3>名称空间<br />Namespaces</h3><ul>';
    ...
    if (members.globals.length) {
        ...
        if (!globalNav) {
            ...
            nav += '<h3>全局<br />Global</h3><ul>' + globalNav + '</ul>';
        }
    }
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    Object.keys(helper.longnameToUrl).forEach(function(longname) {
        ...
        if (myClasses.length) {
            generate('类(Class): ' + myClasses[0].name, myClasses, helper.longnameToUrl[longname]);
        }
        
        var myModules = helper.find(modules, {longname: longname});
        if (myModules.length) {
            generate('模块(Module): ' + myModules[0].name, myModules, helper.longnameToUrl[longname]);
        }

        var myNamespaces = helper.find(namespaces, {longname: longname});
        if (myNamespaces.length) {
            generate('名称空间(Namespace): ' + myNamespaces[0].name, myNamespaces, helper.longnameToUrl[longname]);
        }
    ...
    function saveChildren(node) {
        node.children.forEach(function(child) {
            generateTutorial('教程(Tutorial): ' + child.title, child, helper.tutorialToUrl(child.name));
            saveChildren(child);
        });
    }
